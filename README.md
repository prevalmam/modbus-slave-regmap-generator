...
本ツールでは，Excel でこの6つのレジスタに同じグループ名を指定しておけば，今回書き込まれた分は新しい値，書き込まれなかった分は RAM の現在値を自動的に組み合わせて，検証関数へ渡すコードが生成されます。アプリ側は，書き込み範囲がどうであれ常に「6つそろった1つの日時」として値を受け取れるので，「今回どの項目が書かれたか」を気にせずに判定ロジックだけを書けます。
