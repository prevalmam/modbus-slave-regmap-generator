...
class WriteGuardGenerationTests(unittest.TestCase):
    ...
    def test_generates_typed_callbacks_pointer_snapshot_and_busy_api(self):
        ...
        self.assertIn("modbus_string_view_t name;", header)
        self.assertIn("const char *data;", header)
        self.assertIn("uint16_t length;", header)
        ...
        self.assertIn("current_value.length = modbus_string_field_length(", parser_source)
        self.assertIn("snapshot->name.length = modbus_string_field_length(", parser_source)
    ...
    def test_accepts_excel_boolean_cells(self):
        ...
        self.assertFalse(entry["update_notify"])
